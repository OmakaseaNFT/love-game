...
export const fetchLovePriceUSDT = async (usdtLovePoolContract: PoolAbi) => {
  ...
  const LOVEReserves = Number(
    ethers.utils.formatUnits(USDTLOVEReserves[LOVEKey], 18)
  );
  const USDTReserves = Number(
    ethers.utils.formatUnits(USDTLOVEReserves[USDTKey], 6)
  );
...
export const fetchLovePriceETH = async (ethLovePoolContract: PoolAbi) => {
  ...
  const LOVEReserves = Number(
    ethers.utils.formatUnits(ETHLOVEReserves[LOVEKey], 18)
  );
  const ETHReserves = Number(
    ethers.utils.formatUnits(ETHLOVEReserves[ETHKey], 18)
  );
...
export const fetchWarPriceLove = async (warLovePoolContract: PoolAbi) => {
  ...
  const LOVEReserves = Number(
    ethers.utils.formatUnits(WARLOVEReserves[LOVEKey], 18)
  );
  const WARReserves = Number(
    ethers.utils.formatUnits(WARLOVEReserves[WARKey], 18)
  );
...
export const fetchWarPriceUSDT = async (
...
export const calculateStakedLiquidity = async (
  ...
  if (isLovePair) {
    ...
    const LOVEInFarmBN = lpTotalSupply.isZero()
      ? lpTotalSupply
      : LOVEReservesBN.mul(lpBalanceFarm).div(lpTotalSupply);
    ...
    const WARInFarmBN = lpTotalSupply.isZero()
      ? lpTotalSupply
      : WARReservesBN.mul(lpBalanceFarm).div(lpTotalSupply);
...
export const calculateAPR = async (poolIndex: number) => {
  ...
  const totalLiquidityLocked: ethers.BigNumber = await lpContract.balanceOf(
  ...
  if (isLovePair) {
    ...
    const totalLoveLockedBN = lpTotalSupply.isZero()
      ? lpTotalSupply
      : totalLiquidityLocked.mul(LOVEReservesBN).div(lpTotalSupply);
    const totalLoveLocked = Number(
      ethers.utils.formatUnits(totalLoveLockedBN, 18)
    );
    ...
    const annualRewardInToken = Number(
      ethers.utils.formatUnits(annualRewardInTokenBN, 18)
    );
    ...
    const WARPerBlock = Math.round(
      Number(ethers.utils.formatUnits(lovePerBlock, 18)) * WARPriceLove
    );
    ...
    const totalWarLockedBN = lpTotalSupply.isZero()
      ? lpTotalSupply
      : WARReservesBN.mul(totalLiquidityLocked).div(lpTotalSupply);
    const totalWarLocked = Number(
      ethers.utils.formatUnits(totalWarLockedBN, 18)
    );
    const totalValueLocked = totalWarLocked * 2;

    const annualRewardInTokenBN = WARPerBlockBN.mul(blocksPerYearBN)
      .mul(allocPointBN)
      .div(totalAllocPointBN);
